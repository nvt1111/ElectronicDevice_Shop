...
const applyCoupon = async (req, res, next) => {
    ...
    const productNames = req.body.productNames[0].split(',').map(item => item.trim());;
    const quantities = req.body.quantities[0].split(',').map(item => item.trim());;
    ...
    const coupon = await Coupon.findOne({ name: req.body.nameCoupon }).populate('user');
    if (!coupon) {
    ...
    res.render('checkout', { totalPriceCheck, check, totalPrice: currentTotalPrice, userName, productNames, quantities, prices, user_id, isLoggedIn, user });
